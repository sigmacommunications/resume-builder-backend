...
{
    /**
     * POST /api/templates/{template}/buy
     * Body (optional for paid): payment_method, currency, meta
     */
    public function buy(Request $request, Template $template)
    {
        $validator = Validator::make($request->all(), [
            'stripe_token' => 'required|string', // yeh token frontend se aayega
        ...
        if ($template->status !== 'active') {
            return response()->json(['success' => false, 'message' => 'Template is not available.'], 400);
        }

        $amount   = (float) ($template->price ?? 0);
        if ($amount <= 0) {
            return response()->json(['success' => false, 'message' => 'This template is free, no need to pay.'], 400);
        }
        ...
        try {
            ...
            $charge = Charge::create([
                ...
                "description" => "Template purchase (ID: {$template->id}) by User ".Auth::id(),
            ]);
            ...
            return response()->json([
                'success' => true,
                'message' => 'Payment successful. Template unlocked!',
